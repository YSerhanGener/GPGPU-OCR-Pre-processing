...
__global__ void tiled_rgb2gray_inversion_noise_removal_histogram(uint8_t *input, uint8_t *output, int WIDTH, int HEIGHT, unsigned int* bins, unsigned int numPixels, int trial){
	...
	for(int t=0; t<trial; t++){
		...
		// ) pad the shared memory based on the tile coordinates
		if(ROW >= 0 && ROW < HEIGHT && COL >= 0 && COL < WIDTH){
			TILE[threadIdx.y][threadIdx.x] = 255 - (input[start+indexI]*0.21 // Red value of the input array
						+ input[start+indexI+1]*0.71 // Green value of the input array
						+ input[start+indexI+2]*0.07);//input[ROW * WIDTH + COL];
		}
		else{
			TILE[threadIdx.y][threadIdx.x] = 0;
		}
		...
		if(threadIdx.y < TILE_SIZE && threadIdx.x < TILE_SIZE/* && ROW < HEIGHT && COL < WIDTH*/){
			...
			if(ROW_out < HEIGHT && COL_out < WIDTH){
				int currPixel=0;
				currPixel=floorf(result);
				output[start+ROW_out * WIDTH + COL_out] = currPixel;
				if (gid < numPixels){
				...
				if(tid < NUM_BINS){
					atomicAdd(&bins[(t*NUM_BINS) + tid],subHist[tid]);
				}
			}
		}
	}
...
__global__ void shared_opening(uint8_t *input, uint8_t *output, int WIDTH, int HEIGHT, int trial){
	...
	for(int t=0; t<trial; t++){
		...
		__shared__ uint8_t Ns[BLOCK_SIZE][BLOCK_SIZE];
		if(start_row >= 0 && start_row < HEIGHT && start_col >= 0 && start_col < WIDTH){
			Ns[threadIdx.y][threadIdx.x]= input[start+start_row * WIDTH + start_col];	
		}
		else{
			Ns[threadIdx.y][threadIdx.x]=255;
		}
		...
		if(threadIdx.y < LOW_TILE_SIZE && threadIdx.x < LOW_TILE_SIZE){
			...
			if(NEW_ROW < HEIGHT && NEW_COL < WIDTH){
				output[start+NEW_ROW * WIDTH + NEW_COL] = maximum;
			}
		}
	}
}
...
__global__ void rotate_gather_90(uint8_t *src, uint8_t *dst, unsigned int new_width, unsigned int new_height, int trial) {
	int start = 0;
	for(int t=0; t<trial; t++){
		start = t*new_width*new_height;
		...
		if(x_dst<new_width && y_dst<new_height){
			// Copy pixel
			dst[start+y_dst * new_width + x_dst] = src[start+y_src * width + x_src];
		}
	}
}
...
__global__ void rescaling_bilin(const uint8_t *src, uint8_t *dst, const unsigned int src_width, const unsigned int src_height, const float scale, const unsigned int dst_width, const unsigned int dst_height, int trial) {
	int src_start = 0;
	int dst_start = 0;
	for(int t=0; t<trial; t++){
		src_start = t*src_width*src_height;
		dst_start = t*dst_width*dst_height;
		...
		if ((abs(x_src_off) < (src_width / 2.0))
			...
			top_bilin_intrpl = src[src_start+(unsigned int)(floor(y_src) * src_width + ceil(x_src))] * rght_frac;
			top_bilin_intrpl += src[src_start+(unsigned int)(floor(y_src) * src_width + floor(x_src))] * left_frac;
	
			float bot_bilin_intrpl;
			bot_bilin_intrpl = src[src_start+(unsigned int)(ceil(y_src) * src_width + floor(x_src))] * rght_frac;
			bot_bilin_intrpl += src[src_start+(unsigned int)(ceil(y_src) * src_width + ceil(x_src))] * left_frac;
	
			uint8_t bilin_intrpl;
			bilin_intrpl = (uint8_t)((float)(top_bilin_intrpl * top_frac) + (bot_bilin_intrpl * bot_frac));
	
			dst[dst_start+y_dst * dst_width + x_dst] = bilin_intrpl;
		} else if ((x_dst < dst_width) && (y_dst < dst_height)) {
			dst[dst_start+y_dst * dst_width + x_dst] = 0;
		}
	}
}
...
__global__ void adaptive_threshold_calculation(unsigned int* bins, unsigned int numPixels, uint8_t* threshold, int trial){
	int start = 0;
	for(int t=0; t<trial; t++){
		start = t*NUM_BINS;
		...
		float zeroVal = (float)bins[start+tid]/(float)numPixels;
...
__global__ void adaptive_threshold_apply(uint8_t *inImage, uint8_t *outImage, int numPixels, uint8_t* threshold, int trial) {
	int start = 0;
	for(int t=0; t<trial; t++){
		start = t*numPixels;
		...
		if (i < numPixels){
			...
			outImage[start+i] = __vcmpgeu2(inImage[start+i], threshold[t]);
		}
	}
}
